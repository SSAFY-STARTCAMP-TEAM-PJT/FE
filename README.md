<p align="center">
  <img src="docs/logo.svg" width="120" alt="LocalHub 로고" />
</p>
...
- **지도 조회 최적화**: 뷰포트 확장 조회와 TTL·LRU 캐시를 적용해 지도 이동 시 발생하는 중복 API 요청을 줄였습니다.
- **안정적인 비동기 처리**: 디바운스와 `AbortController`를 활용해 불필요한 요청을 취소하고 이전 응답이 최신 화면을 덮는 문제를 방지했습니다.
- **URL 기반 상태 관리**: 검색어, 카테고리, 선택한 여행지를 쿼리스트링과 동기화해 새로고침과 링크 공유 후에도 상태를 복원합니다.
- **API 응답 정규화**: Backend 응답을 서비스 계층에서 일관된 Frontend 모델로 변환해 컴포넌트와 API 스펙의 결합도를 낮췄습니다.
